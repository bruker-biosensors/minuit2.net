...
{
    protected override Func<double, IList<double>, double> Model { get; } = (x, c) => c[0] + c[1] * x + c[2] * x * x;
    protected override Func<double, IList<double>, IList<double>> ModelGradient { get; } = (x, _) => [1, x, x * x];
    protected override IReadOnlyCollection<string> ParameterNames { get; } = ["c0", "c1", "c2"];
