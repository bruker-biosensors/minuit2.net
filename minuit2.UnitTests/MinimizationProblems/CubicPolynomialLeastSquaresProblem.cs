...
internal class CubicPolynomialLeastSquaresProblem : ConfigurableLeastSquaresProblem
{
    protected override Func<double, IList<double>, double> Model { get; } =
        (x, c) => c[0] + c[1] * x + c[2] * x * x + c[3] * x * x * x;
    
    protected override Func<double, IList<double>, IList<double>> ModelGradient { get; } =
        (x, _) => [1, x, x * x, x * x * x];
    
    protected override IReadOnlyCollection<string> ParameterNames { get; } = ["c0", "c1", "c2", "c3"];
    ...
    protected override IReadOnlyCollection<double> OptimumParameterValues { get; } = [9.97, -1.96, 0.99, -0.1];

    protected override IReadOnlyCollection<double> DefaultInitialParameterValues { get; } = [10.75, -1.97, 1.13, -0.11];
}